...
#         # assert that object is displayed in result
...
#     def test_delete_correct_parent_and_reverse_perms_but_can_only_read_parent(self):
#         p.set_perms_shortcut(self.user, Brand, "w", field_name="products")
#         p.set_perms_shortcut(self.user, Brand, "r")
#         p.set_perms_shortcut(self.user, Product, "w", field_name="brand")
#         resp = self.user_client.delete("/brand/1/products", data=[99], content_type="application/json")
...
#         self.assertEquals(0, len(data["objects"]))
#         self.assertEquals(None, Product.objects.get(id=99).brand_id)
#         # assert that no objects are displayed in result
...
#         self.assertEquals(0, len(data["objects"]))
    ...
    #     self.assertDictEqual( {"id": 3, "barcode": "product_3", "brand_id": 1}, data["objects"][0])