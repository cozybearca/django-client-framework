...
class TestPostCollection(TestCase):
    ...
    def test_post_brand(self):
        ...
        obj = resp.json()
        self.assertDictContainsSubset(obj, {"id": 101, "name": "test_brand"})
        self.assertTrue(Brand.objects.filter(name="test_brand").exists())

    def test_post_brand_2(self):
        self.assertEquals(100, Brand.objects.count())
    ...
    def test_post(self):
        ...
        resp = self.superuser_client.post(
            "/product", {"barcode": "unique", "brand_id": 1}
        )
    ...
    def test_post_invalid_fk(self):
        self.assertEquals(0, Product.objects.count())
        resp = self.superuser_client.post(
            "/product", {"barcode": "unique", "brand_id": 200}
        )
        self.assertEquals(0, Product.objects.count())
        self.assertDictEqual(
            resp.json(), {"brand_id": 'Invalid pk "200" - object does not exist.'}
        )
