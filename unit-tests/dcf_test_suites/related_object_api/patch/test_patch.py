...
class TestRetrieve(TestCase):
    def setUp(self):
        ...
        self.products = [Product.objects.create(barcode=f"product_{i+1}", brand=self.brand) for i in range(100)]

    def test_patch_success(self):
        resp = self.superuser_client.patch("/product/1/brand", data=[ 2 ], content_type='application/json')
        data = resp.json()
        self.assertEqual(2, Product.objects.get(id=1).brand_id)


    def test_patch_failed_invalid_fk(self):
        resp = self.superuser_client.patch("/product/1/brand", data=[23], content_type='application/json')
        data = resp.json()
        self.assertDictEqual(data, { "brand_id": 'Invalid pk "23" - object does not exist.' })


    def test_patch_failed_multiple_ids(self):
        resp = self.superuser_client.patch("/product/1/brand", data=[23, 24], content_type='application/json')
        data = resp.json()
